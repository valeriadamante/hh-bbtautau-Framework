...
def GetBTagWeightDict(var, all_histograms, want2D=False, wantBTag=False):
    all_histograms_1D = {}
    final_json_dict = {}
    for sample_type in all_histograms.keys():
        #print(sample_type)
        ...
        for key_name,histogram in all_histograms[sample_type].items():
            ...
            if want2D:
                for yBin in range(0, histogram.GetNbinsY()+1):
                    histName = f"{sample_type}_{ch}_{reg}_{cat}_{uncName}{scale}"
                    ...
                    if sample_type != "data" and cat not in ['boosted','inclusive'] and var not in var_to_add_boosted:
                        ...
                        histName_num = f"{sample_type}_{ch}_{reg}_{cat}_{uncName}{scale}_num"
                        hist1DProjection_num = ratio_num_hist.ProjectionX(f"{histName_num}_pfx", yBin, yBin)
                        histName_den = f"{sample_type}_{ch}_{reg}_{cat}_{uncName}{scale}_den"
                        ...
                        final_json_dict[sample_type][second_key]['yBin'].append(yBin)
                        final_json_dict[sample_type][second_key]['nJets'].append(ratio_num_hist.GetYaxis().GetBinCenter(yBin))
                ...
                if not histlist:
                    all_histograms_1D[sample_type][key_name]= hist1D
                else:
                    final_hist =  histlist[0]
                    objsToMerge = ROOT.TList()
                    for hist in histlist[1:]:
                        objsToMerge.Add(hist)
                    final_hist.Merge(objsToMerge)
                    all_histograms_1D[sample_type][key_name] = final_hist
            else:
...
if __name__ == "__main__":
    ...
    all_histograms_1D,final_json_dict=GetBTagWeightDict(args.var,all_histograms,False, False)
